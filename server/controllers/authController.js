  ...
  if (!cognitoClient) {
    ...
    try {
      ...
      cognitoClient = new CognitoIdentityProviderClient({
        region: region,
        credentials: {
          accessKeyId: await getParameterValue("/n11725605/prac-accessKeyId"), // Parameter에서 accessKeyId 가져오기
          secretAccessKey: await getParameterValue(
            "/n11725605/prac-secretAccessKey"
          ), // Parameter에서 secretAccessKey 가져오기
          sessionToken:
            (await getParameterValue("/n11725605/prac-sessionToken")) || "", // Parameter에서 sessionToken 가져오기 (optional)
        },
      });
...
const showLoginPage = async (req, res) => {
  try {
    const clientId = await getParameterValue("/n11725605/COGNITO_CLIENT_ID");
    const redirectUri = await getParameterValue(
      "/n11725605/COGNITO_REDIRECT_URI"
    );
...
const registerUser = async (req, res) => {
  const { email, password } = req.body;
  const cognitoClientId = await getParameterValue(
    "/n11725605/COGNITO_CLIENT_ID"
  ...
  try {
    ...
    if (err.code === "InvalidPasswordException") {
      req.flash(
        "error_msg",
        "Password did not meet the policy requirements. It must have at least one symbol, uppercase letter, lowercase letter, and number."
      );
    } else {
