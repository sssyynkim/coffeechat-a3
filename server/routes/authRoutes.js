...
const initializeCognito = async () => {
  try {
    ...
    clientId = await getParameterValue("/n11725605/COGNITO_CLIENT_ID");
    redirectUri = await getParameterValue("/n11725605/COGNITO_REDIRECT_URI");
    clientSecret = await getParameterValue("/n11725605/GOOGLE_CLIENT_SECRET"); // Secrets 대신 Parameter로 전환
...
router.get("/auth/google", async (req, res) => {
  try {
    ...
    const cognitoLoginUrl = `https://${cognitoDomain}/oauth2/authorize?identity_provider=Google&redirect_uri=${encodeURIComponent(
      redirectUri
    )}&response_type=CODE&client_id=${clientId}&scope=email+openid+profile&state=${state}`;
...
router.get("/callback", async (req, res) => {
  ...
  try {
    const clientId = await getParameterValue("/n11725605/COGNITO_CLIENT_ID");
    const clientSecret = await getParameterValue(
      "/n11725605/GOOGLE_CLIENT_SECRET"
    );
    const redirectUri = await getParameterValue(
      "/n11725605/COGNITO_REDIRECT_URI"
    );
